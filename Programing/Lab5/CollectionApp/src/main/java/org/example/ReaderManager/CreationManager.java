...
public class CreationManager {
    ...
    public  Worker creationWorker(int id,String element,boolean hasElement){
        if (!hasElement){
            ...
            element = this.inputValidator.execute(String.format("Insert the data new data for the element id %d %n",id), new ValidationString(), String::trim);
        }
        else{
            System.out.printf("Insert the data new data for the element %s %n",element);
        }
        Position position = this.inputValidator.execute(String.format(">> Insert position %s: ", Arrays.toString(Position.values())), new ValidationEnum<>(Position.class,true), Position::getFromString);
        Status status = this.inputValidator.execute(String.format(">> Insert status %s: ", Arrays.toString(Status.values())), new ValidationEnum<>(Status.class,true), Status::getFromString);
    ...
    private Address createAddress(){
        String zipCode = this.inputValidator.execute(">> Insert the Address: ", new ValidationString(9), String::trim);
