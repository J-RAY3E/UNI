...
public final class Worker implements  Comparable<Worker>, Serializable {
    ...
    public String getEndDateStr() {
        return this.endDate == null ? "N/A": new SimpleDateFormat("yyyy-MM-dd").format(this.endDate);
    }
    ...
    public String getOrganizationStr() {
        return this.organization==null ? "N/A" : this.organization.toString();
    }

    public String getPositionStr() {
        return this.position==null ? "N/A" : this.position.toString();
    }
    ...
    public String getInfo() {
        return String.format(
                "ID: %-10s  Name: %-20s%n" +
                        "Coordinates: %-25s  Salary: $%-10s%n" +
                        "Position: %-15s  Status: %-15s%n" +
                        "Organization: %s%n" +
                        "Creation Date: %-15s  End Date: %-15s%n" +
                        "WhoCreated: %s %n" +
                        "===============================================%n",
                this.getIdStr(), this.getName(),
                this.getCoordinates(), this.getSalaryStr(),
                this.getPositionStr(), this.getStatusStr(),
                this.getOrganizationStr(),
                this.getCreationDate().toLocalDate(), this.getEndDateStr(),
                this.getWhoModificates());
    }
    ...
    public String toCommandString() {
        ...
        if (coordinates != null) {
            sb.append(" :coords:");
            sb.append(coordinates.getX() != null ? coordinates.getX() : "N/A");
            sb.append("$").append(coordinates.getY());
        } else {
            sb.append(" :null:N/A$N/A");
        }
        if (organization != null) {
            sb.append(" ").append(organization.toCommandString());
        }
        else {
            sb.append(" :null:N/A$N/A");
        }
